...
def update_transcribations(file_name, dst):
	#print('update_transcribations', file_name, dst)
	ms_sql_conn = connect_mssql()
	with ms_sql_conn:
		local_cursor = ms_sql_conn.cursor()
		...
		#	print(str(e), str(dst), str(filename))
...
def main():
	...
	with ms_sql_conn:
		...
		for row in cursor.fetchall():
			...
			step += 1
	print(datetime.datetime.now(), 'job complete. exit')
